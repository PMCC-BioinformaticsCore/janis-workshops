...
class Gatk4DataPreprocessing(Workflow):
    ...
    def constructor(self):
        ...
        AlignSubworkflow = self.create_alignment_subworkflow()
        self.step(
            "align",
            AlignSubworkflow(
        ...
        self.step(
            ...
            Gatk4ApplyBqsr_4_1_4(
                ...
                intervalStrings=self.create_sequence_groupings.sequence_grouping_with_unmapped,
            ),
            scatter="intervalStrings",
        )
    ...
    @staticmethod
    def create_alignment_subworkflow():

        wf = WorkflowBuilder("align_subworkflow")

        wf.input("sample_name", str)
        wf.input("reads", FastqGzPair)
        wf.input("ref_fasta", FastaWithIndexes)
        wf.input("compression_level", Int)
        ...
        wf.output("output_bam", source=wf.bwamem.out)
...
class CreateSequenceGroupings(PythonTool):
    ...
    def outputs(self):
        return [
            ...
            TOutput("sequence_grouping_with_unmapped", Array(Array(String))),
        ]


if __name__ == "__main__":
    Gatk4DataPreprocessing().translate("wdl")
